...
public:
    ...
    int externalPathLength() const 
    {
        int sum = 0;
        externalPathLengthHelper(root, sum);
        return sum;
    }
private:
    int countInternalNodesHelper(const shared_ptr<Node>& node) const 
    {
    ...
    int countNodesHelper(const shared_ptr<Node>& node) const 
    {
        if (node == nullptr) 
        {
            return 0;
    ...
    void externalPathLengthHelper(const shared_ptr<Node>& node, int& sum) const 
    {
        if (node == nullptr) {
            return;
        }
        ...
        externalPathLengthHelper(node->left, sum);
        externalPathLengthHelper(node->right, sum);
    }
};
